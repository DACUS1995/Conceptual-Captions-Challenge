...
def positional_encoding(position, d_model):
    ...
    # sines, cosines shape (position, d_model/2)

    # concat second dimension
    # pos_encoding shape (position, model)
    pos_encoding = np.concatenate([sines, cosines], axis=-1)

    # pos_encoding shape (1, position, model)
...
class TransformerWrapper():
    ...
    def create_masks(self, inp, tar):
        ...
        # dec_target_padding_mask shape (batch, 1, 1, seq_len)
    ...
    @tf.function
    def val_step(self, inp, tar, metrics):
        for metric in metrics:
            metrics[metric].reset_states()
    ...
    def evaluate(self, image, max_length, attention_features_shape, input_resize_dim, tokenizer):
        ...
        for i in range(max_length):
            ...
            predicted_word = tokenizer.index_word[predicted_id.numpy().ravel()[0]]
